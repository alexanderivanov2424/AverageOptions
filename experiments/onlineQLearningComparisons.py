...
def make_option_agent(mdp, nx_graph, Matrix, intToS, method='eigen'):
    ...
    return OptionAgent(method + "-options", mdp.actions, call_back, default_q=0.0, option_q=0.0, alpha=.1, online=True)
...
def make_plot(dom, task="", rand_init_and_goal=True, n_options=8, add_options_n_ep=50, n_instances=10, n_episodes=100, n_steps=500):
    ...
    filename = f'online_pesimistic_{exp_name}_inst{n_instances}_ep{n_episodes}_op{n_options}'
